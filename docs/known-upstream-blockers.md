...
| [BeeUI #549](https://github.com/beobungbu/BeeUI/issues/549) | BeeECOM PR #33; exact failing head `5e693a98d01d55159f449876a497154ce9755e39`, CI `34419564897` | `useBeeToken('motion.normal')` on Web | Confirmed runtime mismatch: token reader expects `ms`, Uniwind returns `.2s` from `--motion-duration-normal` | Quarantine only the `motion.*` runtime probe so scoped theme/CSS acceptance can continue. Preserve the failing evidence and re-enable the same motion assertion after consuming a BeeUI release containing #549. |
| [BeeUI #550](https://github.com/beobungbu/BeeUI/issues/550) | BeeECOM PR #33; exact failing head `2db08a82d7b9011da7f692253b57f2cf60af2319`, CI `34424257202` | `useBeeToken` inside `BeeThemeScope` on a real Web consumer | Confirmed public-contract mismatch: scope-aware hook reads return the global theme in Chromium | Quarantine only scope-aware hook assertions. Continue validating `BeeThemeScope` itself through computed semantic CSS, nested/sibling isolation, scope changes and Web Popover portal preservation. Re-enable the hook assertions after consuming a BeeUI release containing #550. |
